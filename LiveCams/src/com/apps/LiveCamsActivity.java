...
public class LiveCamsActivity extends Activity {
    ...
    private void setupMainGridView(){
        ...
        gridViewAdapter=new SimpleAdapter(this,gridViewList,R.layout.main_item,
    ...
    private void loginNoticeProcess(MessageParam msgparam){
    	if(msgparam.flag[msgparam.RESULT]){
			if(1==msgparam.result || 400==msgparam.result){
				...
				String srvtype=!msgparam.flag[msgparam.SRVTYPE]?
					"服务器":(0==msgparam.srvtype?"服务器":
    			...
    			if(rec.cmd.equals(msgparam.cmd)){
    				if("request_uploadsnap".equals(rec.cmd)){
    					if(1==msgparam.result){
	    					...
	    					DispatchHandler.submitShortNotice("提示：成功上传抓拍照片("+(String)rec.object+	")!");
    					}else{
	    					Log.v(TAG,"上传抓拍照片("+(String)rec.object+")失败!");
	    					DispatchHandler.submitLongNotice("警告：上传抓拍照片("+(String)rec.object+")失败!");   						
    					}
    					...
    					if(1==msgparam.result){
    						...
    						Log.v(TAG,"成功上传"+(String)rec.object+"("+
    					...
    					if(1==msgparam.result){
        					if(msgparam.flag[msgparam.RTVLIST]){
        						...
        						if(rtvcnt>0){
        							...
        							msg.what=252;
        							msg.arg1=rtvcnt;
        							msg.arg2=SettingAndStatus.isplaymode?0:(SettingAndStatus.isrecordmode?1:-1);
        							msg.obj=rtvlist;
        							handler.sendMessage(msg);
        							isok=true;
        						}else{
    					...
    					if(1==msgparam.result){
    						if(msgparam.flag[msgparam.PORT]){
	    							...
	    							new Thread(new Runnable(){
	    				        		public void run(){
			    			        		...
			    			        		if(SettingAndStatus.settings.avrecord){
			    								...
			    								if(!arecorder.open("LiveCams",filename)){
			    									arecorder.close();
			    									arecorder=null;
	    						...
	    						Log.v(TAG,"成功播放"+(String)rec.object+"("+
	    								Utils.intToIpaddr(SettingAndStatus.settings.srvipaddr)+":"+
	    								msgparam.port+")!");
								...
								for(int i=0,offset=0;i<msgparam.PICMAXCNT;i++){
									if(msgparam.piclen[i]>0){
										if(0==msgparam.pictype[i] || 1==msgparam.pictype[i]){
											...
											if(pic.open("LiveCams",filename+"-"+i+".jpg")){
    				...
    				DispatchHandler.submitLongNotice("警告：序号为"+msgparam.seqno+"的命令("+msgparam.cmd+")处理异常("+
   						(-1==msgparam.errno?"应答超时":(-2==msgparam.errno?"应答包过大":
						(-3==msgparam.errno?"消息堵塞":(-4==msgparam.errno?"内存不足":"未知原因"))))+")!");	    
    				it.remove();
    			}
    		}
    	}
			...
			if(Vca.getMessage(msg,1000)){
				...
				if(NoticeId.NOTICE==msg.type){
					...
					if(msgparam.parserCmdAck(param.getBytes())){
						Log.v(TAG,"收到一个"+(CmdId.ACK==msgparam.id?
							"应答":"命令")+"消息:\n"+param);						
						switch(msgparam.id){
						case CmdId.ACK:
    ...
    private boolean openAndStartVca(View v){
		...
		vcaParam.dispheight=SettingAndStatus.settings.fullscreen?
			SettingAndStatus.displaywidth:0;
		vcaParam.dispwidth=SettingAndStatus.settings.fullscreen?
			SettingAndStatus.displayheight:0;
