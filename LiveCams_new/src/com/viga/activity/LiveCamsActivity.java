...
public class LiveCamsActivity extends Activity {
	...
	private static LiveCamsActivity mSingleton;
	...
	private void quitApp() {
		AlertDialog dialog = new AlertDialog.Builder(LiveCamsActivity.this).setTitle("提示")
	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		if (1 == requestCode && 1 == resultCode) {
			...
			Vca.Arg arg = vca.new Arg(-1, -1);   
			Vca.control(Vca.OPID_CLRSRC, arg);
		}else if(6 == requestCode && 6== resultCode){
			if (SettingAndStatus.phoneDataSending) {
				SettingAndStatus.phoneDataSending = false;
				CmdSend.stopSendData(false);
		...
		//本地视频播放
		else if(7 == requestCode && 7== resultCode){
			if (SettingAndStatus.phoneDataSending) {
				SettingAndStatus.phoneDataSending = false;
				CmdSend.stopSendData(false);
				DataProc.stopVideoUpload();
	...
	private void setupMainGridView() {
		...
		ib_login.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				if (!flag_login) {
					ib_login.setImageDrawable(getResources().getDrawable(
							R.drawable.v5_2_1_desktop_list_friends));
					flag_login = true;
				} else {
					ib_login.setImageDrawable(getResources().getDrawable(
							R.drawable.v5_2_1_desktop_list_friends_blue));
					flag_login = false;
				}
	...
	/* 打开和启动VCA模块 */
	 boolean openAndStartVca(View v) {
		 Log.i(TAG,"打开和启动VCA模块");
		boolean ret = false;
	...
	private class GvItemClickListener implements OnItemClickListener {
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
			...
			case 4:// 在线播放
				...
				View mPlay=mainGridView.getChildAt(4);
				avPlayClickProcess(mPlay);
				break;
			case 5:// 本地上传
			...
			if (0 == SelectRtvDialog.this.mode) {
				...
				for (int i = 0; i < SelectRtvDialog.this.count; i++) {
					...
					case 0:
						...
						Log.v("YangQiang","服务器音频");
						break;
					case 1:
					case 2:
						Log.v("YangQiang","背负音频");
						bt[1].setEnabled(true);
						bt[1].setOnClickListener(new View.OnClickListener() {
							public void onClick(View v) {
								String name = getString(R.string.frontvideo);
