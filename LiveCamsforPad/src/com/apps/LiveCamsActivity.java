...
public class LiveCamsActivity extends Activity {
	...
	private void setupMainGridView() {
		...
		gridViewAdapter = new SimpleAdapter(this, gridViewList,
				R.layout.main_item, new String[] { "ItemImage", "ItemText" },
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case 252:/* 弹出让用户选择播放实时视频对话框 */
				...
				new SelectRtvDialog(LiveCamsActivity.this, rtvlist, rtvcnt,
						mode).show();
				break;
			default:
	...
	private void loginNoticeProcess(MessageParam msgparam) {
		if (msgparam.flag[msgparam.RESULT]) {
			if (1 == msgparam.result || 400 == msgparam.result) {
				...
				String srvtype = !msgparam.flag[msgparam.SRVTYPE] ? "服务器"
						: (0 == msgparam.srvtype ? "服务器"
	...
	private void disconnectedNoticeProcess(MessageParam msgparam) {
		...
		SettingAndStatus
				.insertLogRecord(SettingAndStatus.loginTime, new Date());
			...
			while (it.hasNext()) {
				...
				if (rec.seqno == msgparam.seqno) {
					if ("request_uploadsnap".equals(rec.cmd)) {
						...
						DispatchHandler.submitLongNotice("警告：上传抓拍照片("
								+ (String) rec.object + ")失败!");
	...
	private void hbtimeoutNoticeProcess(MessageParam msgparam) {
		if (msgparam.flag[msgparam.TIMECNT]) {
			...
			DispatchHandler.submitLongNotice("警告：心跳回复超时!\n(心跳频率："
					+ vcaParam.hbperiod + "秒;超时门限：" + vcaParam.hbtimeout
					+ "秒;超时时间：" + msgparam.timecnt + "秒)");
		} else {
				...
				if (rec.cmd.equals(msgparam.cmd)) {
					if ("request_uploadsnap".equals(rec.cmd)) {
						if (1 == msgparam.result) {
							Log.v(TAG, "成功上传抓拍照片(" + (String) rec.object + ")!");
							DispatchHandler.submitShortNotice("提示：成功上传抓拍照片("
									+ (String) rec.object + ")!");
						} else {
							Log.v(TAG, "上传抓拍照片(" + (String) rec.object + ")失败!");
							DispatchHandler.submitLongNotice("警告：上传抓拍照片("
						...
						if (1 == msgparam.result) {
							...
							Log.v(TAG,
									"成功上传"
											...
											+ ":" + msgparam.port + ")!");
							DispatchHandler.submitShortNotice("提示：成功上传"
									+ (String) rec.object + "!");
						} else {
							Log.v(TAG, "上传" + (String) rec.object + "失败!");
							DispatchHandler.submitShortNotice("警告：上传"
									+ (String) rec.object + "失败!");
						}
						it.remove();
					} else if ("request_getrtvlist".equals(rec.cmd)) {
							...
							if (msgparam.flag[msgparam.RTVLIST]) {
								...
								if (rtvcnt > 0) {
									...
									msg.arg2 = SettingAndStatus.isplaymode ? 0
											: (SettingAndStatus.isrecordmode ? 1
													: -1);
									msg.obj = rtvlist;
									handler.sendMessage(msg);
									isok = true;
									...
									DispatchHandler
											.submitShortNotice("警告：实时播放列表中无有效视频!");
								}
							} else {
								Log.v(TAG, "实时播放列表为空!");
								DispatchHandler
										.submitShortNotice("警告：实时播放列表为空!");
							}
						} else {
							...
							if (msgparam.flag[msgparam.PORT]) {
								...
								if (SettingAndStatus.settings.audioduplex1) {
									new Thread(new Runnable() {
										public void run() {
											// 发送请求上传数据命令
											CmdSend.requestSendData(0, true,
													false, true);
											// 准备录音文件
											...
											if (SettingAndStatus.settings.avrecord) {
												...
												if (!arecorder.open("LiveCams",
														filename)) {
													arecorder.close();
													arecorder = null;
													Log.v(TAG, "无法录音，请检查SD卡!");
													DispatchHandler
															.submitLongNotice("警告：无法录像，请检查SD卡!");
												} else {
													String fullname = arecorder
															.getFileFullName();
													Log.v(TAG, "开始录音("
															+ fullname + "!");
													DispatchHandler
															.submitShortNotice("提示：开始录音("
																	+ fullname
																	+ ")!");
												}
											}
								...
								Log.v(TAG,
										"成功播放"
												...
												+ Utils.intToIpaddr(SettingAndStatus.settings.srvipaddr)
												+ ":" + msgparam.port + ")!");
								DispatchHandler.submitShortNotice("提示：成功播放"
										+ (String) rec.object + "!");
							} else {
								Log.v(TAG, "无效应答消息!");
								DispatchHandler.submitShortNotice("警告：播放"
										+ (String) rec.object + "失败!");
							}
						} else {
						...
						if (1 == msgparam.result) {
							if (msgparam.flag[msgparam.PICCNT]
									&& msgparam.piccnt > 0
									&& msgparam.flag[msgparam.PICTYPE]
									&& msgparam.flag[msgparam.PICLEN]) {
								for (int i = 0, offset = 0; i < msgparam.PICMAXCNT; i++) {
									if (msgparam.piclen[i] > 0) {
										if (0 == msgparam.pictype[i]
												|| 1 == msgparam.pictype[i]) {
											...
											if (pic.open("LiveCams", filename
													+ "-" + i + ".jpg")) {
												pic.write(message.data, offset,
														msgparam.piclen[i]);
												pic.close();
											}
										}
								...
								Log.v(TAG, "无效应答消息!");
								DispatchHandler
										.submitShortNotice("警告：通知背负上传抓拍图片失败!");
							}
						} else {
					...
					DispatchHandler
							.submitLongNotice("警告：序号为"
									+ msgparam.seqno
									+ "的命令("
									+ msgparam.cmd
									+ ")处理异常("
									+ (-1 == msgparam.errno ? "应答超时"
											: (-2 == msgparam.errno ? "应答包过大"
													: (-3 == msgparam.errno ? "消息堵塞"
															: (-4 == msgparam.errno ? "内存不足"
																	: "未知原因"))))
									+ ")!");
					it.remove();
				}
			...
			if (Vca.getMessage(msg, 1000)) {
				...
				if (NoticeId.NOTICE == msg.type) {
					...
					if (msgparam.parserCmdAck(param.getBytes())) {
						Log.v(TAG, "收到一个"
								+ (CmdId.ACK == msgparam.id ? "应答" : "命令")
								+ "消息:\n" + param);
						switch (msgparam.id) {
						case CmdId.ACK:
							if (msgparam.flag[msgparam.SEQNO]
									&& msgparam.flag[msgparam.CMD]
	...
	private boolean openAndStartVca(View v) {
		...
		vcaParam.dispheight = SettingAndStatus.settings.fullscreen ? SettingAndStatus.displaywidth
				: 0;
		vcaParam.dispwidth = SettingAndStatus.settings.fullscreen ? SettingAndStatus.displayheight
				: 0;
		if (Vca.open(vcaParam)) {
			...
			if (Vca.start(40, SettingAndStatus.settings.sndport, 1024 * 1024,
					SettingAndStatus.settings.rcvport, 1024 * 1024,
					SettingAndStatus.settings.timeout)) {
				SettingAndStatus.vcaStatus.status = SettingAndStatus.VcaStatus.STARTED;
	...
	private void startPlayActivity(int id) {
		Intent intent = new Intent(LiveCamsActivity.this,
			...
			if (SettingAndStatus.vcaStatus.status >= SettingAndStatus.VcaStatus.STOPPING) {
				...
				if (SettingAndStatus.settings.logoutcheck) {
					AlertDialog dialog = new AlertDialog.Builder(
							LiveCamsActivity.this)
							.setMessage(getString(R.string.logoutcheck))
							.setCancelable(false)
							.setPositiveButton("确 认",
									new DialogInterface.OnClickListener() {
										public void onClick(
												DialogInterface dialog, int id) {
											SettingAndStatus.insertLogRecord(
													SettingAndStatus.loginTime,
													new Date());
											stopAndCloseVca();
										}
									})
	...
	private class GvItemClickListener implements OnItemClickListener {
		public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
			...
			switch (arg2) {
			case 0:// 参数设置
				Log.v(TAG, "点击了【" + getString(R.string.setting).toString()
						+ "】项！");
				settingClickProcess();
				break;
			case 1:// 系统登陆
				...
				break;
			case 2:// 手机拍照
				Log.v(TAG, "点击了【" + getString(R.string.snappic).toString()
						+ "】项！");
				snapPicClickProcess();
				break;
			case 3:// 手机录像
				Log.v(TAG, "点击了【" + getString(R.string.record).toString()
						+ "】项！");
			...
			case 4:// 在线播放
				...
				break;
			case 5:// 本地上传
				Log.v(TAG, "点击了【" + getString(R.string.upldpic).toString()
						+ "】项！");
				break;
			case 6:// 本地浏览
				Log.v(TAG, "点击了【" + getString(R.string.browser).toString()
						+ "】项！");
				nativeScanClickprocess();
				break;
			case 7:// 设备控制
				Log.v(TAG, "点击了【" + getString(R.string.devctrl).toString()
						+ "】项！");
				break;
			default:
				break;
	...
	private class SelectRtvDialog extends Dialog {
		...
		public void onCreate(Bundle savedInstanceState) {
			...
			((Button) findViewById(R.id.cancelBt))
